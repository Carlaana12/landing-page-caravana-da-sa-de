...
const VideoUploader: React.FC<VideoUploaderProps> = ({ onUpload }) => {
  ...
  const handleUpload = async (file: File) => {
    try {
      ...
      onUpload(url);
      toast.success('Vídeo/Áudio enviado com sucesso');
    } catch (error) {
      toast.error('Erro ao enviar vídeo/áudio');
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="flex flex-col items-center">
        {uploading ? (
          ...
          <>
            ...
            <span className="text-sm text-gray-500">Clique para enviar vídeo ou áudio</span>
          </>
        )}
      </div>
    </label>
